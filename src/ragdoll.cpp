...
public:
    ...
    void createScene() override
    {
        ...
        def.mass = 3;
        def.shape = edyn::box_shape{0.17, 0.07, 0.1};
        def.update_inertia();
        ...
        // Left upper leg
        def.mass = 8;
        def.shape = edyn::box_shape{0.075, 0.205, 0.075};
        def.update_inertia();
        def.position = {0.0985, 1.8, 0};
        ...
        // Left lower leg
        def.mass = 7;
        def.shape = edyn::box_shape{0.06, 0.205, 0.06};
        def.update_inertia();
        def.position = {0.0985, 1.39, 0};
        ...
        def.update_inertia();
        def.position = {0.0985, 1.185, -0.06};
        ...
        // Lower back
        def.mass = 5;
        def.shape = edyn::box_shape{0.155, 0.065, 0.086};
        def.update_inertia();
        def.position = {0, 2.1354, 0};
        auto lower_back = edyn::make_rigidbody(*m_registry, def);

        // Mid back
        def.mass = 6;
        def.shape = edyn::box_shape{0.151, 0.084, 0.07};
        def.update_inertia();
        def.position = {0, 2.27, 0};
        auto mid_back = edyn::make_rigidbody(*m_registry, def);

        // Chest
        def.mass = 7;
        def.shape = edyn::box_shape{0.17, 0.108, 0.095};
        def.update_inertia();
        def.position = {0, 2.4485, 0};
        auto chest = edyn::make_rigidbody(*m_registry, def);

        // Neck
        def.mass = 2;
        def.shape = edyn::box_shape{0.06, 0.065, 0.06};
        def.update_inertia();
        def.position = {0, 2.6, 0};
        auto neck = edyn::make_rigidbody(*m_registry, def);

        // Head
        def.mass = 4;
        def.shape = edyn::box_shape{0.075, 0.09, 0.105};
        def.update_inertia();
        def.position = {0, 2.7437, -0.025};
        auto head = edyn::make_rigidbody(*m_registry, def);

        // Left shoulder
        def.mass = 1.5;
        def.shape = {};
        def.inertia = edyn::diagonal_matrix(edyn::moment_of_inertia_solid_box(def.mass, {0.1, 0.1, 0.1}));
        def.position = {0.1105, 2.507, 0};
        ...
        // Left upper arm
        def.mass = 2.5;
        def.shape = edyn::box_shape{0.135, 0.05, 0.05};
        def.update_inertia();
        def.position = {0.3055, 2.507, 0};
        ...
        // Left lower arm
        def.mass = 1;
        def.shape = edyn::box_shape{0.135, 0.04, 0.04};
        def.update_inertia();
        def.position = {0.5755, 2.507, 0};
        def.orientation = edyn::quaternion_identity;
        ...
        def.update_inertia();
        def.position = {0.776, 2.502, -0.005};
        def.orientation = edyn::quaternion_identity;
        ...
        // Hip - Upper legs
        for (auto leg : std::array{leg_lu, leg_ru}) {
            edyn::scalar side = leg == leg_lu ? 1 : -1;
            auto cone_rot =
            ...
            cone_con.pivot[0] = {side * edyn::scalar(0.0985), 0, 0};
            cone_con.pivot[1] = {0, -0.205, 0};
            ...
            cvjoint.pivot[0] = {side * edyn::scalar(0.0985), 0, 0};
            cvjoint.pivot[1] = {0, 0.205, 0};
            ...
            cvjoint.twist_min = edyn::to_radians(leg == leg_lu ? -80 : -15);
            cvjoint.twist_max = edyn::to_radians(leg == leg_lu ? 15 : 80);
        ...
        for (auto legs : std::array{std::make_pair(leg_ru, leg_rl), std::make_pair(leg_lu, leg_ll)}) {
            ...
            hinge.pivot[0] = {0, -0.205, 0};
            hinge.pivot[1] = {0, 0.205, 0};
        ...
        // Hip-lower back
        {
            ...
            cone_con.pivot[0] = {0, 0.07, 0};
            cone_con.pivot[1] = {0, 0.065, 0};
            ...
            cvjoint.pivot[0] = {0, 0.07, 0};
            cvjoint.pivot[1] = {0, -0.065, 0};
            cvjoint.frame[0] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cvjoint.frame[1] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
        ...
        // Mid back-lower back
        {
            ...
            cone_con.pivot[0] = {0, 0.065, 0};
            cone_con.pivot[1] = {0, 0.084, 0};
            cone_con.frame = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            ...
            cvjoint.pivot[0] = {0, 0.065, 0};
            cvjoint.pivot[1] = {0, -0.084, 0};
            cvjoint.frame[0] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cvjoint.frame[1] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
        ...
        // Mid back-chest
        {
            ...
            cone_con.pivot[0] = {0, 0.084, 0};
            cone_con.pivot[1] = {0, 0.108, 0};
            cone_con.frame = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            ...
            cvjoint.pivot[0] = {0, 0.084, 0};
            cvjoint.pivot[1] = {0, -0.108, 0};
            cvjoint.frame[0] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cvjoint.frame[1] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
        ...
        // Neck-head
        {
            ...
            cone_con.pivot[0] = {0, 0.04, 0};
            cone_con.pivot[1] = {0, 0.18, 0.025};
            cone_con.frame = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cone_con.span_tan[0] = std::tan(edyn::to_radians(16));
            cone_con.span_tan[1] = std::tan(edyn::to_radians(32));
            cone_con.bump_stop_stiffness = 5000;
            ...
            cvjoint.pivot[0] = {0, 0.04, 0};
            cvjoint.pivot[1] = {0, -0.09, 0.025};
            cvjoint.frame[0] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cvjoint.frame[1] = edyn::matrix3x3_columns(edyn::vector3_y, -edyn::vector3_x, edyn::vector3_z);
            cvjoint.twist_min = edyn::to_radians(-30);
            cvjoint.twist_max = -cvjoint.twist_min;
        ...
        for (auto shoulder : std::array{shoulder_l, shoulder_r}) {
            ...
            cone_con.pivot[0] = {edyn::scalar(0.05005) * side, 0.0585, 0};
            ...
            cvjoint.pivot[0] = {edyn::scalar(0.05005) * side, 0.0585, 0};
            cvjoint.pivot[1] = {-0.06, 0, 0};
        ...
        for (auto pair : std::array{std::make_pair(shoulder_l, arm_lu), std::make_pair(shoulder_r, arm_ru)}) {
            ...
            cone_con.pivot[0] = {0.06, 0, 0};
            cone_con.pivot[1] = {0.135, 0, 0};
            cone_con.frame = edyn::matrix3x3_columns(
                edyn::rotate(cone_rot, edyn::vector3_x),
                edyn::rotate(cone_rot, edyn::vector3_y),
                edyn::rotate(cone_rot, edyn::vector3_z));
            ...
            cvjoint.pivot[0] = {0.06, 0, 0};
            cvjoint.pivot[1] = {-0.135, 0, 0};
        ...
        for (auto arms : std::array{std::make_pair(arm_lu, arm_ll), std::make_pair(arm_ru, arm_rl)}) {
            ...
            hinge.pivot[0] = {0.135, 0, 0};
            hinge.pivot[1] = {-0.135, 0, 0};
        ...
        for (auto pair : std::array{std::make_pair(forearm_l, hand_l), std::make_pair(forearm_r, hand_r)}) {
            auto [cone_ent, cone_con] = edyn::make_constraint<edyn::cone_constraint>(*m_registry, pair.first, pair.second);
            cone_con.pivot[0] = {0.135, 0, 0};
            cone_con.pivot[1] = {0.13, 0.005, 0.005};
